...
template<typename T>
void mymemcpy(thrust::host_vector<T, std::allocator<T> > &H,
...
template<typename T>
void mymemcpy(thrust::device_vector<T, thrust::device_malloc_allocator<T> > &D,
...
template<typename T>
void mymemcpy(thrust::device_vector<T, thrust::device_malloc_allocator<T> > &Do,
...
template<typename T>
void mymemcpy(thrust::device_vector<T, thrust::device_allocator<T> > &Do,
...
template<typename T>
void mymemzero(thrust::device_vector<T, thrust::device_malloc_allocator<T> >& D) {
    int dev_num;
    cudaGetDevice(&dev_num);
    cudaMemsetAsync(thrust::raw_pointer_cast(D.data()), 0, sizeof(T)*D.size(), cuda_stream[dev_num]);
}
template<typename T>
void mymemzero(thrust::device_vector<T, thrust::device_allocator<T> >& D) {
    int dev_num;
    cudaGetDevice(&dev_num);
    cudaMemsetAsync(thrust::raw_pointer_cast(D.data()), 0, sizeof(T)*D.size(), cuda_stream[dev_num]);
}
