...
void calculate_distances(int n, int d, int k,
    ...
    /*
    std::cout <<"diatance :"<<std::endl;
    for(int i = 0;i < n;i++){
    	for(int j = 0;j < k;j++){
    		std::cout <<pairwise_distances[i*k + j] << " ";
    	}
    }
    std::cout <<std::endl;
    */
...
__global__ void make_new_labels(int n, int k, double* pairwise_distances,
    ...
    if (global_id < n) {
        ...
        if (old_label != min_idx) {
            atomicAdd(changes, 1);//maybe use default
        }
    }
}
...
void cub_init() {
    ...
    cub_initted = true;
    //std::cout <<"CUB init end..."<<std::endl;
}
void cub_close() {
	//std::cout <<"CUB close" << std::endl;
    for (int q=0; q<16; q++) {
    ...
    cub_initted = false;
    //std::cout <<"CUB close end..."<<std::endl;
}
